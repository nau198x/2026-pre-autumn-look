...
description: LP のデザイントークン（CSS カスタムプロパティ）・ブレークポイント・タイポグラフィの標準辞書。`:root` トークンの完全雛形、過去案件で実際に起きた命名ドリフト（--color-rule vs --color-border / --color-muted の役割混同 / コンテンツ幅トークン 3 系統 / --ease-out の中身違い 3 種）の「揺れ実例 → 標準名」対応表、mobile-first ブレークポイント標準と旧 max-width 方式の 1px 境界バグ警告、欧文ディスプレイ + 和文明朝のフォント構成と clamp() 流体タイプのレシピ、min(%, px) 幅プリセット方式までを含む。ユーザーが「デザイントークン決めて」「CSS 変数を整理して」「:root の変数作って」「ブレークポイントどうする」「レスポンシブの境界値」「フォント設定して」「タイポグラフィ組んで」「clamp でフォントサイズ」「色と余白の変数」「トークン初期化して」等を言ったとき、または新規 LP プロジェクトの立ち上げ・スタイル基盤整備・他案件からの CSS 移植の文脈では必ずこのスキルを使うこと。
---
...
6. 移植した CSS 内の揺れ名を対応表に従って標準名へ一括置換し、未定義変数参照が残っていないか検査する
